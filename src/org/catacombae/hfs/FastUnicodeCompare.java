...
public class FastUnicodeCompare {
	...
	char c1, c2;
	char temp;
	int[] lowerCaseTable = gLowerCaseTable;
	...
	while(true) {
		...
		c1 = str1[str1ptr++];//*(str1++);
		--length1;
		if((temp = (char)lowerCaseTable[c1>>8]) != 0)         //  is there a subtable
		    //  for this upper byte?
		    c1 = (char)lowerCaseTable[temp + (c1 & 0x00FF)];  //  yes, so fold the char
	    }
	    //  Find next non-ignorable char from str2, or zero if no more
	    while(length2 != 0 && c2 == 0) {
		c2 = str2[str2ptr++];//*(str2++);
		--length2;
		if((temp = (char)lowerCaseTable[c2>>8]) != 0)         //  is there a subtable
		    //  for this upper byte?
		    c2 = (char)lowerCaseTable[temp + (c2 & 0x00FF)];  //  yes, so fold the char
	    }
	    if(c1 != c2)        //  found a difference, so stop looping
		break;
	    else if(c1 == 0)    //  did we reach the end of both strings at the same time?
		return 0;       //  yes, so strings are equal
	}
	if(c1 < c2)
	    return -1;
